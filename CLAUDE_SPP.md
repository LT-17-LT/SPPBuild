...
- DO build all placeholders as specified in the brief — no Seedance video, 3D golf balls, or putt video exist yet. Use CSS/SVG placeholders that match the final animation logic so assets can be swapped in later without refactoring.
- DO build the hero word animation as scroll-driven (not timed to a video). The video layer is a separate concern that gets added later.
- DO build golf balls as SVG circles with dimple pattern and radial gradient. These are production-quality stand-ins.
- DO build the putt green as a CSS gradient surface with SVG ball and hole.
- DO respect prefers-reduced-motion.
- DO use GPU-accelerated transforms only (translate, scale, opacity).
- DO preload the hero video asset when it exists (placeholder doesn't need preload).

...
**Asset required:** Seedance video (MP4, 1920×1080). NOT YET AVAILABLE.

**PLACEHOLDER (build this now):**
- Full-viewport dark gradient background:
  `radial-gradient(ellipse 70% 50% at 50% 45%, rgba(23,63,34,0.35) 0%, transparent 70%), linear-gradient(170deg, #050d07 0%, #0a1f10 40%, #09140b 100%)`
- Subtle animated glow that slowly drifts (CSS animation, 8s loop, transforms only)
- Centred product silhouette image at ~30% opacity (use existing Wix product PNG: `https://static.wixstatic.com/media/14555f_9ee6f490554c44588c309928951a6dbc~mv2.png`)
- Silhouette scales very slightly on scroll (1.0 → 1.05 over full scroll range) for subtle parallax life
- The word animation and scroll behaviour should work identically to the final version — only the background visual changes when the real video arrives
- When Seedance video is ready: swap the placeholder div for a `<video>` tag, wire `video.currentTime = scrollProgress * video.duration`
...
**PLACEHOLDER (build this now — no 3D assets yet):**
- CSS/SVG golf balls: white circles with the characteristic dimple pattern
- Build each ball as an SVG circle (~40–200px) with a subtle radial gradient (white → light grey) and a faint dimple texture overlay (tiny circles or dots pattern at low opacity)
- 15–25 balls scattered across the section at randomised positions, sizes (40px–200px) and opacities (0.15–0.7)
- CSS animation: each ball drifts downward at a different speed (2–8s loop, translateY), slight horizontal wobble
- Parallax on scroll: balls at different z-depths move at different scroll rates via GSAP
- One ball in the lower-centre is slightly larger and more opaque — this is the "hero ball" that will separate and become the thread
- These are production-quality placeholders — when real 3D renders arrive, swap the SVG for PNGs but keep the same animation logic
...
**Transition out:** The hero ball separates from the group, begins its descent. The rest fade. This single ball becomes the scroll-driven visual thread.
...
- Single SVG golf ball (same style as the hero ball from Section 2), scroll-position driven
- Falls vertically as user scrolls (GSAP ScrollTrigger, translateY mapped to scroll progress)
- Subtle CSS rotation on the ball (rotate transform, continuous)
...
- Ball is the same SVG element repositioned — not a new instance
...
**Background:** Transitions from paper to a putting green visual
**Ball behaviour:**
...
**PLACEHOLDER (build this now — no video/3D assets yet):**
- Green surface: CSS gradient simulating a top-down putting green (`--green` → `--green2`, subtle noise texture if possible via SVG filter)
- Hole/cup: dark circle with subtle inner shadow, centred or slightly off-centre
- Ball: same SVG golf ball used throughout the site (visual consistency)
- Scroll-driven animation: ball translateY + slight translateX curve toward the hole, scale down slightly as it "sinks" (0.95 → 0.3 → 0 opacity at the cup)
- Optional: very subtle shadow under the ball that shrinks as it approaches the hole
- This section should be ~60vh tall — enough scroll room for the roll to feel satisfying
- When real video/3D assets arrive: replace the CSS green + SVG ball with a proper rendered animation, keeping the same scroll-trigger timing
